...
public class UnpaidBillsService {
    ...
    public ResponseEntity<?> UnpaidBills(List<String> productIds, int discount, String customer) {
        try {
            ...
            for (String productId : productIds) {
                Products products = productRepo.findById(productId).orElse(null);
                if (products != null) {
                    totalAmount += products.getPrice();

                    // Create a ReceiptProduct object with the product details
                    ReceiptProduct receiptProduct = new ReceiptProduct(products.getPrdName(), products.getPrice());
